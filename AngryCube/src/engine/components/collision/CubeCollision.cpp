...
CubeCollision::CubeCollision(GameObject* parent)
	: Collision(parent)
{
}

std::shared_ptr<GameObjectComponent> CubeCollision::Clone() const
{
	return std::make_shared<CubeCollision>(*this);
}

std::shared_ptr<GameObjectComponent> CubeCollision::MoveClone()
{
	return std::make_shared<CubeCollision>(std::move(*this));
}

void CubeCollision::Enable()
{
	enabled = true;
}

void CubeCollision::Disable()
{
	enabled = false;
}

bool CubeCollision::Enabled() const
{
	return enabled;
}

bool CubeCollision::IsColliding(std::shared_ptr<GameObject> other) const
{
	if (!CanCollide(other))
		return false;
...
void CubeCollision::ResolveCollision(std::shared_ptr<GameObject> penetrated)
{
...
glm::vec2 CubeCollision::CalculateDistanceToObject(std::shared_ptr<GameObject> other) const
{
	if (!CanCollide(other))
		return glm::vec2();
...
glm::vec2 CubeCollision::CalculateDistanceBetweenCenters(std::shared_ptr<GameObject> other) const
{
	if (!CanCollide(other))
		return glm::vec2();
...
glm::vec2 CubeCollision::CalculateCollisionPoint(std::shared_ptr<GameObject> penetrated) const
{
	if (!CanCollide(penetrated))
		return glm::vec2();
...
bool CubeCollision::CanCollide(std::shared_ptr<GameObject> other) const
{
...
bool CubeCollision::DidHitSide(std::shared_ptr<GameObject> penetrated, glm::vec2 collisionPoint) const
{
	if (!CanCollide(penetrated))
		return false;
...
float CubeCollision::CalculateClosestSideX(glm::vec2 center, float halfWidth, glm::vec2 v) const
{
	float x0 = center.x;
	if (v.x < 0)
		x0 += halfWidth;
...
float CubeCollision::CalculateClosestSideY(glm::vec2 center, float halfHeight, glm::vec2 v) const
{
	float y0 = center.y;
	if (v.y < 0)
		y0 += halfHeight;
