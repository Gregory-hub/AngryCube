...
void DragFloatWithSetter(const char* label, T* obj, Getter getter, Setter setter, float speed = 1.0f, float min = 0.0f, float max = 0.0f, const char* format = "%.3f") {
    // 1. Get the current value from the getter
    float currentValue = (obj->*getter)();
