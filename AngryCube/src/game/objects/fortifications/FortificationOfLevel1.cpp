    ...
    : Fortification(parentScene)
{
    ...
    brickLowest->GetTransform().SetTranslation(glm::vec2(0.0f, brickLowest->GetCollisionMesh()->GetHeight() / 2.0f + padding));
    ...
    auto brickHighest = brickTemplate->Clone();
    brickHighest->GetTransform().SetTranslation(brickHigh->GetTransform().GetTranslation() +
        glm::vec2(0.0f, brickHighest->GetCollisionMesh()->GetHeight() + padding));
    AttachChild(brickHighest, false);
}
